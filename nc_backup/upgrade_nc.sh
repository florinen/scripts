...
CURRDATE=$( date '+%m-%d-%Y' )
NC_FOLDER="nextcloud"
NC_TARGET_VER="18.0.4"
DIR="$HOME/old_nc"
NC_LOCATION="/var/www"
DOWNLOAD_NC="curl -LO https://download.nextcloud.com/server/releases/nextcloud"

NC_CURR_VER=$(cat /var/www/nextcloud/config/config.php | grep version | awk '{print $3}' | sed "s/['\,,\"]//g" | cut -b -6)
if [[ $NC_CURR_VER != $NC_TARGET_VER ]]; then
    echo "You are about to upgrade NC to $GREEN >> $NC_TARGET_VER <<$RESET...!!!"
    if [[ $NC_CURR_VER != $NC_TARGET_VER ]]; then
        $DOWNLOAD_NC-$NC_TARGET_VER.zip ;
        unzip $NC_FOLDER-$NC_TARGET_VER.zip &>/dev/null ;
        rm $NC_FOLDER-$NC_TARGET_VER.zip
    fi
else
    echo "You DON'T need to upgrade, NC version $YELLOW >> $NC_TARGET_VER is a match with $NC_CURR_VER <<$RESET...!!!"
    exit
fi
...
# if [[ $(cat /var/www/nextcloud/config/config.php | grep version | awk '{print $3}' | sed "s/['\,,\"]//g" | cut -b -6) == $NC_TARGET_VER ]]; then
#     new_ver=$(cat /var/www/nextcloud/config/config.php | grep version | awk '{print $3}' | sed "s/['\,,\"]//g" | cut -b -6)
#     echo "....$GREEN NC successfuly upgraded new version is $new_ver $RESET...."
#     exit
# elif [[ $(cat /var/www/nextcloud/config/config.php | grep version | awk '{print $3}' | sed "s/['\,,\"]//g" | cut -b -6) != $NC_TARGET_VER ]]; then
