...
foreach ($site in $sites) {
    ...
    $report = $vms | ForEach-Object {
	 $ips = @()
	 $_.guest.net.ipaddress | ForEach-Object {
	   if ($_) {
	     $ips += $_
	   }
	 }
	...
	$report | Sort-Object Host | export-csv -path "C:\Users\Desktop\esxi\logs\$site $((Get-Date).ToString('MM-dd-yyyy_hhmm')).csv" -NoTypeInformation -UseCulture
...
#Cleanup old csv after 7 days.  Delete files older than the limit
 ...
 $path = "C:\Users\Desktop\esxi\logs\"
